            ...
            <List item-layout="vertical">
                <ListItem v-for="item in mainBlogItem" :key="item.id">
                    <a @click="inBlog(item.id)">
                        <ListItemMeta :avatar="item.avatar" :title="item.uname" :description="item.title" />
                    </a>
                    <template slot="action">
                        ...
                        <li @click="like(item.id)">
                            <Icon type="ios-thumbs-up-outline" /> {{item.likeNum}}
                        </li>
                        <li>
                            <Icon /> {{item.btime}}
                        </li>
    ...
    <i-col span="8" offset="1">
        <row>
            ...
            <Card style="">
                ...
                <Tag class="tag" @click.native="getBlogList()">显示全部</Tag>
                <Tag class="tag" @click.native="getlableInfo(item.tag)" :color="bgColor[index%12]" v-for="(item,index) in tag" :key="index">{{item.tag}}</Tag>
            </Card>
        </row>
        <row>
